...
producer="$repo/scripts/fixtures/PersonalizedFixtureGenerator.java"
default_output="$repo/TestSupport/Fixtures/API/Recommendations/personalized_cross_language.pb"
...
temporary_root="$(mktemp -d "${TMPDIR:-/tmp}/tiebalite-jvm-fixture.XXXXXX")"
...
protoc \
  ...
  --descriptor_set_out="$temporary_root/personalized.desc" \
  --include_imports \
  Personalized.proto
javac -cp "$jar" -d "$temporary_root/classes" "$producer"
java -cp "$temporary_root/classes:$jar" \
  PersonalizedFixtureGenerator \
  "$temporary_root/personalized.desc" \
  "$output"

printf 'Generated JVM cross-language fixture: %s\n' "$output"
